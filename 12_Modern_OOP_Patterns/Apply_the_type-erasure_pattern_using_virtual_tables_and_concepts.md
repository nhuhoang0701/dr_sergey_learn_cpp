...
void erasure_approach() {
    ...
    shapes.push_back(Circle{5.0});
    shapes.push_back(Square{3.0});
    // shapes.push_back(42);  // ❌ compile error: int has no .draw()
...
- **`std::function` uses this pattern** — small lambdas (≤16-32 bytes typically) are stored in an internal buffer; large ones are heap-allocated.
