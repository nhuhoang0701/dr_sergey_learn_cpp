...
There is a fundamental asymmetry in how braced-init-lists (`{1, 2, 3}`) interact with template deduction vs `auto`:
...
The standard gives `auto` a special rule: when the initializer is a braced-init-list, `auto` deduces `std::initializer_list<T>` (C++11/14) or `T` for single element (C++17).
