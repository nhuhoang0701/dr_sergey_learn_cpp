...
When the compiler substitutes template arguments, it processes them **left to right** in the template parameter list and then through the function signature. A failure in one parameter can affect whether later parameters are well-formed.
...
- `requires` clause is **always** in the immediate context → no accidental hard errors
- Error messages name the failing constraint
- Supports subsumption for overload ordering
- Cleaner than `enable_if` in every position
