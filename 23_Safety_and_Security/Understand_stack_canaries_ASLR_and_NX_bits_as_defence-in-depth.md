...
Stack canaries, ASLR, and NX bits are **compiler/OS-level mitigations** that make buffer overflow exploits significantly harder — but don't eliminate the root cause. Together they form a **defence-in-depth** strategy: each blocks a different exploitation technique, so an attacker must bypass all of them.
