...
Rate-Monotonic Analysis (RMA) provides a sufficient schedulability test: for N periodic tasks, the system is schedulable if total utilization $U = \sum_{i=1}^{N} \frac{C_i}{T_i} \leq N(2^{1/N} - 1)$, which converges to $\ln(2) \approx 0.693$ as $N \to \infty$.
...
// Sufficient condition: U = Σ(Ci/Ti) ≤ N(2^(1/N) - 1)
...
// RM utilization bound: N(2^(1/N) - 1) × 1,000,000
// Precomputed for N=1..8 (converges to ln(2)≈693,147 ppm)
...
constexpr std::array<TaskSpec, 4> system_tasks = {{
    {"imu_read",    1'000,   150,  1'000, 4},  // 1 kHz, 150 μs WCET
    {"pid_control", 2'000,   300,  2'000, 3},  // 500 Hz, 300 μs WCET
    {"can_tx",      5'000,   200,  5'000, 2},  // 200 Hz, 200 μs WCET
    {"telemetry",  10'000,   500, 10'000, 1},  // 100 Hz, 500 μs WCET
}};
