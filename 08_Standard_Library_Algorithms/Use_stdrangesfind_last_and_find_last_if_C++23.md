...
`std::ranges::find_last`, `find_last_if`, and `find_last_if_not` (C++23) search for the **last** matching element in a range. Before C++23, finding the last occurrence required using reverse iterators or manual backward scanning.
...
Unlike `find` (which returns an iterator), `find_last` returns a **`subrange`** from the found element to the end of the range. If nothing is found, returns an empty subrange at the end.
...
- A split_back function using find_last_if to split on the last delimiter.
