...
C++ provides multiple sorting algorithms optimized for different use cases:
...
- **`std::sort`**: Introsort = quicksort + heapsort fallback + insertion sort for small ranges
- **`std::stable_sort`**: Merge sort (needs O(n) extra memory; falls back to in-place O(n log²n))
- **`std::partial_sort`**: Heapselect — builds a heap of first K elements, then scans the rest
- **`std::nth_element`**: Introselect — quickselect with median-of-medians fallback
...
- `nth_element` runs in **O(n) average** time — it only needs to find where one element goes.
- `sort` does unnecessary work: it fully orders all N elements just to read one.
- After `nth_element`, the element at position `nth` is exactly what it would be in a fully sorted array. All elements before it are ≤, all after are ≥ (like a partition).
- **Use `nth_element` for:** median, percentiles, top-K (unsorted), statistics.

### Q2: Explain why stable_sort preserves relative order and at what complexity cost

**Stability** means that elements comparing equal retain their original relative ordering after sorting.
...
- **Cost of stability:** O(n) extra memory for the merge buffer. If allocation fails, complexity degrades from O(n log n) to O(n log²n).
...
- **Critical:** The comparator must implement **strict weak ordering** (`<`, not `<=`). Using `<=` causes undefined behavior.
...
- **Choosing the right sort:** Full sort → `sort`. Stable → `stable_sort`. Top-K sorted → `partial_sort`. Single position → `nth_element`. Already nearly sorted → `insertion_sort` via `std::sort` (small ranges internally).
- **`std::sort` implementation:** GCC/Clang use introsort: quicksort that falls back to heapsort after log₂(n) recursion depth, plus insertion sort for partitions < 16 elements.
