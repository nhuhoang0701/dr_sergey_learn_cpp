...
These three algorithms take a **predicate** (a callable returning bool) and operate on ranges:
...
All predicates must be **pure functions** — they should not modify their arguments or have side effects (especially with parallel execution policies).
...
`std::remove_if` is an **algorithm** operating on an **iterator range**. It has no knowledge of the container — it only sees iterators. It cannot call container member functions like `erase()` or `resize()`.

What `remove_if` actually does:

1. Moves elements that DON'T match the predicate to the front of the range.
...
- **Predicate must be a pure function:** `count_if`, `copy_if`, and `remove_if` may call the predicate in any order. With parallel policies, it's called concurrently.
