...
`std::transform` writes results through an **output iterator**. By choosing different output iterator adaptors you control *where* the results go — appended to a container, inserted at a position, prepended, or streamed directly to `std::cout`.
...
Writes each assigned value to `os` followed by `delimiter`. Useful for printing transformed results directly without an intermediate container.
...
- `std::back_inserter(upper)` returns a `back_insert_iterator` that calls `upper.push_back()` for each value assigned to it.
- The destination container starts **empty** — no need to pre-size.
- Each call to `push_back` may trigger reallocation. If you know the final size, call `upper.reserve(words.size())` before the transform for efficiency.
...
- `std::ostream_iterator<T>(os, delim)` writes each value to `os` followed by `delim`.
- Works with any `std::ostream` — `std::cout`, `std::ofstream`, `std::ostringstream`.
- Eliminates the need for an intermediate container when you only want to print results.
- Caveat: the delimiter appears **after** every element including the last — you may get a trailing delimiter.
