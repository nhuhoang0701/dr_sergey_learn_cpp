...
`std::shift_left` and `std::shift_right` (C++20) move elements within a range by a specified number of positions. Unlike `std::rotate`, shifted-out elements leave the vacated positions in a valid-but-unspecified (moved-from) state.
