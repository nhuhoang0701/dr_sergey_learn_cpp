...
`decltype` inspects the **declared type** of an entity or the **type and value category** of an expression — without evaluating it.
...
This is the **single most important** thing to understand about `decltype`.
...
`decltype(auto)` (C++14) is a placeholder that applies **decltype rules** to the initializer expression:
...
**Warning:** `decltype(auto) d = (x)` creates a reference! This is a common source of bugs.

### `decltype(auto)` for Return Types

The primary use case is preserving the exact type and value category from a `return` expression:
