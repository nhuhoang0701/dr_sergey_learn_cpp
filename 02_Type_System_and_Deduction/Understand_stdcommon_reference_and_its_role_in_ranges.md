...
The Ranges library requires that iterators and their reference types are **comparable** and **interconvertible**. `common_reference` provides the type that makes this possible.
...
Consider a proxy iterator (like `std::vector<bool>::iterator`):
