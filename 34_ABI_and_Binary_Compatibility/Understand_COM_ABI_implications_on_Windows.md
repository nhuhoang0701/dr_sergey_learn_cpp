...
### Q2: Implement multiple COM interfaces on a single object with correct QueryInterface navigation and prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent prevent reference counting pitfalls
...
- Reference counting in COM is **per-interface, not per-object** in the general case, though most implementations use a single atomic counter for the entire object.
...
- Cross-apartment calls have **~1000x overhead** compared to direct calls due to marshaling — design interfaces to minimize round-trips.
- Use `ComPtr<T>` (from `<wrl/client.h>` or your own) for RAII management of COM pointers — manual `AddRef`/`Release` is the #1 source of COM memory leaks.
