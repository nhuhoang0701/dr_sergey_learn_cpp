...
**Mutual recursion** between lambdas is tricky because each lambda must reference the other, but lambdas are `auto`-typed and one isn't declared yet when the other captures it. The common solutions are:

1. **`std::function`** — type-erased wrapper (works but has overhead)
2. **Trampoline** — pass the other lambda as a parameter (zero overhead)
...
- **Performance:** `std::function` adds ~5-20ns per call due to type erasure. The trampoline approach compiles to the same code as regular function calls.
...
- **Stack depth:** All mutual recursion approaches are subject to stack overflow for deep recursion. Consider tail-call optimization or iterative rewriting.
- **Practical use:** Mutual recursion appears in parsers (expression/term/factor), state machines, and tree traversals.
