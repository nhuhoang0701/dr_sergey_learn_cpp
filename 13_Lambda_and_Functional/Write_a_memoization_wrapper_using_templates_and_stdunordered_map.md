...
**Memoization** caches the results of expensive function calls so that repeated calls with the same arguments return instantly from the cache. A generic `memoize(f)` wrapper uses `std::unordered_map` keyed on the argument tuple.
...
4. **Thread-local caches** — no locking needed, but duplicates work
...
- **Key hashing:** `std::tuple` has no default hash in the standard library — you must provide one.
- **Cache invalidation:** "There are only two hard things in computer science: cache invalidation and naming things." Consider TTL or manual invalidation.
