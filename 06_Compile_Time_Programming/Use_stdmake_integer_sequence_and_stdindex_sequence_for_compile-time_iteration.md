...
`std::index_sequence<Is...>` is a compile-time sequence of `std::size_t` values. It's an alias for `std::integer_sequence<std::size_t, Is...>`.
...
The fundamental pattern: use a helper function that takes `index_sequence<Is...>` as a parameter, then expand a pack expression over `Is...`:
...
- `std::apply` (C++17) uses `index_sequence` internally — it's the standard "tuple → function call" utility.
