...
Problems:
  ① Non-deterministic latency: stack depth determines cost (~1-10 μs)
  ② Code size: .eh_frame tables add 10-15% to binary
  ③ ABI lock-in: exception tables are ABI-specific, hard to change
  ④ Can't use in embedded/RT: many codebases compile with -fno-exceptions
  ⑤ Success path: ACTUALLY zero-cost on Itanium ABI, but some embedded
...
The core idea was to make exceptions behave like return values internally:
...
| Error path overhead | ~1-10 μs (unwinding) | ~10 ns (value return) | ~10 ns (value return) |
...
P0709 was too ambitious for adoption (ABI changes, new keywords, compiler rewrites). Instead, the committee standardized `std::expected<T,E>` which achieves the **deterministic** goal without changing the language:
...
| --- | --- | --- | --- |
| Itanium (table-based) | ~0 ns | ~1-10 μs | +10-15% |
| SJLJ (setjmp/longjmp) | ~20-50 ns per `try` | ~100-500 ns | +5% |
| Windows x64 SEH | ~0 ns | ~1-5 μs | +5-10% |
...
| --- | --- | --- | --- |
| **Ergonomics** | ★★★★★ Automatic propagation | ★★★ Monadic chaining (C++23) | ★ Must check at every call |
| **Happy path cost** | ★★★★★ Zero (Itanium) | ★★★★ Near-zero (discriminant) | ★★★★ Near-zero (branch) |
| **Error path cost** | ★★ ~1-10 μs (unwinding) | ★★★★★ ~10 ns (return value) | ★★★★★ ~10 ns (return value) |
| **Determinism** | ★ Non-deterministic | ★★★★★ Fully deterministic | ★★★★★ Fully deterministic |
| **Binary size** | ★★ +10-15% tables | ★★★★★ Minimal | ★★★★★ Minimal |
| **Caller discipline** | ★★★★★ No discipline needed | ★★★ Must check `.has_value()` | ★ Easy to ignore return value |
| **Composability** | ★★★★ Try/catch blocks | ★★★★ Monadic ops | ★ Manual at every level |
| **Type safety** | ★★★ Any type throwable | ★★★★★ Typed error `E` | ★★ Magic numbers |
| **Embedded/RT** | ❌ Often disabled | ✅ Works everywhere | ✅ Works everywhere |
| **Cross-language** | ❌ C++ only | ⚠️ C++ only (but trivial to convert) | ✅ C-compatible |
