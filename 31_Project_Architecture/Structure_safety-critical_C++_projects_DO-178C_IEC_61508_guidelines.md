...
**Safety-critical software** has regulatory requirements for development processes, code standards, and verification. **DO-178C** governs airborne systems (avionics), **IEC 61508** covers industrial functional safety, and **ISO 26262** targets automotive. These standards mandate structured development, traceability from requirements to tests, restricted language subsets, and formal verification at higher safety integrity levels.
...
- **Traceability** is the #1 requirement: every requirement → design → code → test must be traceable
- **MISRA C++ 2023** is the current language subset standard for safety-critical C++
- **MC/DC coverage** (Modified Condition/Decision Coverage) is required at DO-178C DAL A
- Avoid dynamic memory allocation (`new`/`delete`) in safety-critical code; use static allocation
- Disable exceptions and RTTI in safety-critical builds (`-fno-exceptions -fno-rtti`)
- **Defensive programming**: validate all inputs, check return codes, assert invariants
- Safety-critical code reviews require independence: reviewer must not be the author
- Static analysis tools for safety: Polyspace, Parasoft, PC-lint, LDRA
- Hardware watchdog must be kicked periodically; if software hangs, hardware resets the MCU
