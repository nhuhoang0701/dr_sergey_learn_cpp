...
- **diffoscope** understands 100+ file formats: ELF, archives, zip, PE, Mach-O, PDF, images, etc.
- **`--build-id=sha1`** in GCC generates a build-id based on file content (deterministic) instead of random
- **Bazel** achieves reproducible builds by default through sandboxing and content-addressed caching
- **LLVM's `--build-id=tree`** uses a Merkle tree hash for fast, content-based build IDs
- **Windows:** MSVC's `/Brepro` flag zeros out the PE timestamp header for reproducibility
