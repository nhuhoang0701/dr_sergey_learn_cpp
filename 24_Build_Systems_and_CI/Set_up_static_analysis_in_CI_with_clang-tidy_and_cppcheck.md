...
Static analysis finds bugs, style violations, and potential security issues **without running the code**. Running clang-tidy and cppcheck in CI catches problems before they reach code review. Both tools complement each other: clang-tidy is AST-based (deep analysis), cppcheck uses its own parser (finds different bug classes).
...
# .clang-tidy — project-level configuration
...
- `run-clang-tidy` (with `-j`) parallelizes across all source files — it's faster than running `clang-tidy` in a loop
- `CMAKE_EXPORT_COMPILE_COMMANDS=ON` generates `compile_commands.json`, which tells clang-tidy the exact flags for each file
- Running only on changed files keeps PR feedback fast for large monorepos
- `--warnings-as-errors` makes specific check categories fail the CI job
...
- `--enable=all` enables all checks (style, performance, portability, information, warning)
- `--suppress=missingIncludeSystem` suppresses noisy missing system include warnings
- `--error-exitcode=1` makes cppcheck return exit code 1 if errors found (fails CI)
- `--project=compile_commands.json` uses the exact compile flags from CMake
...
# ═══════════ Per-target clang-tidy (skip third-party code) ═══════════
...
- `CMAKE_CXX_CLANG_TIDY` runs analysis on every compilation — slows the build by 2-4x
- Good for CI (catch everything), bad for local development (too slow)
- Use an option (`ENABLE_CLANG_TIDY`) to keep it off by default, on in CI
- `--fix` flag can auto-fix issues, but don't use in CI (modifies files)

---

## Notes

- **`compile_commands.json`** is required for clang-tidy — always set `CMAKE_EXPORT_COMPILE_COMMANDS=ON`
- **`.clang-tidy`** file in the project root configures checks for the whole project
- **NOLINTBEGIN/NOLINTEND** suppresses clang-tidy for code blocks (e.g., generated code)
- **`iwyu`** (include-what-you-use) is another useful static analysis tool that finds unnecessary `#include`s
- **Combine both tools:** clang-tidy catches modernization and AST-level bugs; cppcheck catches value-flow bugs and leaks. They find different issues.
