...
C++ global/static variables are initialized in two phases:
...
If global `A` in file1.cpp depends on global `B` in file2.cpp, and B hasn't been initialized yet when A's constructor runs → **undefined behavior**.
...
- If `auto_register` initializes before `registry`, it calls `register_item()` on an unconstructed `std::map` → **UB** (crash, corruption, or appears to work).
