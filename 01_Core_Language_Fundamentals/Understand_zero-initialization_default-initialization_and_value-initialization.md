...
C++ has multiple forms of initialization, each with different rules for what value an object gets. Confusing them is a common source of bugs (reading uninitialized memory = UB).
...
Occurs when a variable is declared with **no initializer** in local scope:
...
Occurs with empty braces `{}` or parentheses `()`:
...
Occurs automatically for **static** and **global** variables:
...
When you write `S s{};` for a struct `S` with no user-provided constructor:
