...
This means mismatched format strings — wrong number of arguments, invalid specifiers for the type, or malformed syntax — are compile-time errors, not runtime crashes.
...
Crucially, the `consteval` constructor means the format string must be a compile-time constant (string literal or `constexpr` variable). For runtime-determined format strings, you must use `std::vformat` or `std::runtime_format` (C++26), which defers validation to runtime.
...
**Key insight:** Every `std::format` call runs the format parser at compile time. The `consteval` constructor of `format_string` ensures that if the code compiles, the format string is valid for the given argument types.
...
**Key insight:** By accepting `std::format_string<Args...>` as a parameter, your custom APIs get the same compile-time format validation as `std::format` — zero additional code needed.
...
- Compile-time checking covers: argument count, type compatibility with specifiers, brace matching, positional argument validity.
