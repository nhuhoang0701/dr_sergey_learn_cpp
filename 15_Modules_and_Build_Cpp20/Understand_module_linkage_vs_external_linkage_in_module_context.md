...
**Key insight:** Module linkage entities are mangled with the module name. The linker sees something like `_ZW5audio12mix_channelsPfi` vs `_ZW5video12mix_channelsPfi` (GCC Itanium mangling). This prevents cross-module symbol collisions that would occur with external linkage.
...
To diagnose: use `nm` (Linux/macOS) or `dumpbin /symbols` (MSVC) to inspect object files. Module-linkage symbols will have module-name-qualified mangling.
