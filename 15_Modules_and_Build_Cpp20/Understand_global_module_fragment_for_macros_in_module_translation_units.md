...
The **global module fragment** is a section at the top of a module interface/implementation unit (between `module;` and the module declaration) where you can `#include` legacy headers. This is the **only place** where `#include` is permitted in a module unit while preserving macro access.
...
**Key insight:** Modules create a **firewall** for macros. No matter where you define a macro in a module unit (global fragment or after module declaration), it cannot leak to importers.
