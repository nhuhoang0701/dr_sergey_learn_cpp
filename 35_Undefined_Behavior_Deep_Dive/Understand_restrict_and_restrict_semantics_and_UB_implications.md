...
Here is what a typical `restrict`-annotated function signature looks like:
...
Without `restrict`, the compiler must assume that `out`, `a`, and `b` might overlap (alias). This prevents optimizations because on every iteration the compiler must reload values from memory, not knowing whether a prior write changed them:
...
With `restrict`, the compiler knows no aliasing is possible and can load multiple elements at once with SIMD instructions:
...
If you promise `restrict` but the pointers **do** alias, the behavior is undefined. The compiler will have generated code that assumes non-aliasing, and the results will silently be wrong:
...
They solve related but different problems. It is worth keeping this distinction clear in your head:
...
Here are the two patterns you will see most often. Function parameters are the safest and most portable use:
...
Use Compiler Explorer (Godbolt) to compare generated assembly. You can also ask the compiler directly for its vectorization report:
...
Without `restrict`, you will often see a "can't vectorize: possible aliasing" remark. With `restrict`, the loop vectorizes.
...
All major compilers support `__restrict` as an extension, so it is usable in practice despite not being standard.

### Q2: Show the performance impact of restrict on a SAXPY operation

SAXPY (`y = a * x + y`) is a classic numerical kernel. The contrast between the two versions makes the aliasing problem very concrete:
