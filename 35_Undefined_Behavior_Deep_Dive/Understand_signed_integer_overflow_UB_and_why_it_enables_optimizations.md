...
Because the compiler can assume signed overflow never occurs, it can make powerful optimizations. Here are the five most important patterns to understand:
...
This is where signed overflow UB stops being an academic curiosity and becomes a genuine security problem. Notice how a seemingly defensive check can be silently removed:
...
There are several options depending on your situation. Here they are in order of preference for new code:
...
- Loop optimizations based on "always increasing" assumptions do not apply
...
Integer overflow in size calculations before allocation is one of the most exploited vulnerability classes. Here is a textbook example of how it plays out:
...
- Some loop optimizations are disabled, which can mean slower code
...
- GCC/Clang `-ftrapv` generates traps on overflow (slower than UBSan, but catches issues in production).
